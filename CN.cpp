...
using namespace cv;
using namespace std;
void paint_chinese(Mat& image,int x_offset,int y_offset,unsigned long offset);          //绘制起点坐标
void paint_ascii(Mat& image,int x_offset,int y_offset,unsigned long offset);            //在图片上画汉字
void put_text_to_image(int x_offset,int y_offset,String image_path,char* logo_path);    //将汉字弄上图片，x,y表示起点位置坐标，以图片左上角为0点
int main(){

    String image_path="/home/karmen/ChineseTest/IMAGE.png";        //通过绝对路径定位图像文件
    char* logo_path=(char*)"/home/karmen/ChineseTest/logo.txt";    //通过绝对路径定位文本文件
    put_text_to_image(60,270,image_path,logo_path);
    return 0;
...
void paint_ascii(Mat& image,int x_offset,int y_offset,unsigned long offset){
	...
	//打开ascii字库文件
	FILE *ASCII;
	if ((ASCII = fopen("/home/karmen/ChineseTest/Asci0816.zf", "rb")) == NULL){
		printf("Can't open ASCII.zf,Please check the path!");
		exit(0);
	}
	fseek(ASCII, offset, SEEK_SET);  //移动ASCII的读写指针SEEK_SET偏移offset
	fread(buff, 16, 1, ASCII);       //从ASCII文件流中读取1个16大小的单元到buff中去
	int i, j;
	Point p1 = p;
		...
		for (j = 0; j < 8; j++)              //一个char八个bit
		{
			...
			if (buff[i] & (0x80 >> j))    /*测试当前位是否为1*/
			{
				...
				*/
				...
				circle(image, p1, 0, Scalar(63, 72, 204), -1);
				p1.x++;
...
void paint_chinese(Mat& image,int x_offset,int y_offset,unsigned long offset){//在图片上画汉字
    ...
    if((HZK=fopen("/home/karmen/ChineseTest/HZKs2424.hz","rb"))==NULL){
    ...
    for (i = 0; i < 24; i++)
	{
		...
		{		
			if (mat[i][j])
				circle(image, p, 1, Scalar(167, 132, 171), -1);		  //写(替换)像素点
			p.x++;                                                //右移一个像素点
		}
		p.y++;                                                    //下移一个像素点
	}
